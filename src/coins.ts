...
function drawCards(data:Array<object>) {
    $("#cardcontent").remove()
...
function drawMoreData(data:object,cardId:string) {
    if(!data)return;
    if(!cardId)return;
...
function ajaxgetMoreData(id:string,cardId:string) {
    if(!id)return;
    if(!cardId)return;
...
function card(data:object) {
    ...
    function _isToggleChecked(id:string):boolean {
        if (!id) return;
        if(state.favCoins.includes(id)) return true;
        return false;
    }
...
function addToFav(id:string):void {
    if(!id) return;
...
function removeFromFav(id:string):void {
    if(!id) return;
    if(!state.favCoins.includes(id)) return;
